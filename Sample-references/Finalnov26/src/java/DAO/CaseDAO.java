...
public class CaseDAO {
	...
	public boolean addCase(Case caseDetails) {
		boolean added = false;
		try {
			String query = "insert into cases (name, category, days, statuteNumber) values('" + caseDetails.getName() + "', '"
					+ caseDetails.getCategory() + "', " + caseDetails.getDays() + ", '" + caseDetails.getStatuteNumber() + "')";
	...
	public Case getCaseById(int id) {
		...
		try {
			...
			if (resultSet.next()) {
				...
				caseDetail.setStatuteNumber(resultSet.getString("statuteNumber"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Case getCaseByStatuteNumber(String statuteNumber) {
		Case caseDetail = null;
		try {
			Statement statement = connection.createStatement();
			String query = "select * from cases where statuteNumber = '" + statuteNumber + "'";
			ResultSet resultSet = statement.executeQuery(query);
			if (resultSet.next()) {
				caseDetail = new Case();
				caseDetail.setId(resultSet.getInt("id"));
	...
	public List<Case> getCases() {
		...
		try {
			...
			while (resultSet.next()) {
				...
				caseDetails.setStatuteNumber(resultSet.getString("statuteNumber"));
